...
getTitle :: IO ()
getTitle = do
    print "Input an index:"
    ind <- read <$> getLine
    let book = Map.lookup ind books
    let myTitle = title <$> book
    if isJust myTitle then
        print $ "The title is:\n" ++ fromJust myTitle
    else print $ "could not find the book with index: " ++ show ind
...
-- a list from this type and apply the add1 function from the previous question to it.
